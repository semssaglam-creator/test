...
class Istekci(BaseHTTPRequestHandler):
        ...
        try:
            if yol in ("/", "/index.html"):
                ...
                self._json_yanit({
                    ...
                    # Duzeltme fisi kunye alanlarinin varsayilanlari; arayuz
                    # formu bunlarla doldurur, boylece metinler tek yerde durur
                    "fis_varsayilan": {
    ...
    def _excel_gonder(self, veri):
        ...
        calisma_olustur(dosya_yolu, inceleme, _yillari_coz(calisma), sonuc, bulgular,
                        duzeltmeler)
        self._xlsx_gonder(dosya_yolu, dosya_adi)

    def _duzeltme_fisi_gonder(self, veri):
        """Duzeltme fisi formunu uretir ve indirilmek uzere gonderir.

        Fisin ust blogundaki kunye alanlari (duzenleme nedeni, cilt/sira no,
        tarih, gerekce metni, imza unvanlari) calismayla birlikte saklanir;
        istemci bunlari "fis" anahtariyla gonderir. Verilmeyen alanlar icin
        duzeltme_fisi modulundeki varsayilanlar kullanilir.
        """
        calisma = veri.get("calisma") or {}
        ...
        guvenli = "".join(c for c in (calisma.get("ad") or "kdv")
                          if c.isalnum() or c in " -_").strip() or "kdv"
        dosya_adi = f"KDV_duzeltme_fisi_{guvenli}.xlsx".replace(" ", "_")
        dosya_yolu = os.path.join(CIKTI_DIR, dosya_adi)
        fis_olustur(dosya_yolu, inceleme, _yillari_coz(calisma), sonuc,
                    veri.get("fis") or calisma.get("fis") or {})
        self._xlsx_gonder(dosya_yolu, dosya_adi)

    def _xlsx_gonder(self, dosya_yolu, dosya_adi):
        """Uretilmis bir Excel dosyasini indirme olarak yanitlar."""
