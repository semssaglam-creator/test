    ...
    for anahtar in sorted(gruplar):
        ...
        for sira, b in enumerate(benzersiz):
            ...
            surumler.append({
                ...
                # Devir degisikliginin beyannameye yazilmis gerekcesi;
                # devir sicramasi panelinde gosterilir.
...
def devir_sicramalari(duzen, secim):
    ...
    gerekcesi beyannameye yazilir (eski bicimde "Indirim Nedenleri", 2026
    Nisan sonrasi bicimde "Onceki Donemden Devreden Indirilecek KDV" tablosu).

    Uygulama bu kopuklugu goremezse zinciri kendi hesabiyla tasimaya devam
    eder ve fark, ilerideki bir donemde tespitten dogmamis bir matrah farki
    gibi gorunur. Bu yuzden kopukluk sessizce yutulmaz; kullaniciya bildirilir.

    Karar kullaniciya birakilir. Uc secenek yalnizca devir FAZLA beyan edilmis
    (mukellef lehine sicrama) ve o beyannamede bir gerekce yazilmissa
    sunulur; gerekcesiz bir sicrama icin secenek verilmez, fark oldugu gibi
    kalir.

    Doner: [{"anahtar", "yil", "ay", "etiket", "surum_etiketi",
             "beyan_onceki_devir", "onceki_donem_sonraki_devir", "fark",
             "yon", "indirim_nedenleri", "secenekli"}]
    """
    secilen = secimi_coz(duzen, secim)
    ...
    for onceki, simdiki in zip(sirali, sirali[1:]):
        ...
        tasinan = round(
            float(onceki_surum["degerler"].get("sonraki_donem_devreden") or 0.0), 2)
        beyan = round(
            float(surum["degerler"].get("onceki_donem_devreden") or 0.0), 2)
    ...
    for d in duzen["donemler"]:
        ...
        for s in d["surumler"]:
            ...
            satirlar.append({
                ...
                # Degisen kolonlarin degisim tutari (simdiki - onceki);
                # vurgulanan tutarin altinda "Fark: +/-" olarak gosterilir.
                # Yalnizca degisen kolonlar icin anahtar bulunur.
                "ozet_fark": ozet_fark,
            })
            onceki_surum = s
    return satirlar
