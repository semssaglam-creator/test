@echo off
rem ===========================================================
rem  {{UYGULAMA_ADI}} - masaustu kisayolu olusturur
rem
...
echo.
echo   {{UYGULAMA_ADI}} hazirlaniyor...
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; try { $m=New-Object -ComObject WScript.Shell; $k=$m.CreateShortcut([IO.Path]::Combine($m.SpecialFolders('Desktop'),'{{UYGULAMA_ADI}}.lnk')); $k.TargetPath='%HEDEF%'; $k.WorkingDirectory='%~dp0'; $k.IconLocation='%IKON%'; $k.Description='{{KISAYOL_ACIKLAMA}}'; $k.Save(); exit 0 } catch { Write-Host ('   ' + $_.Exception.Message); exit 1 }"

if errorlevel 1 goto kisayolYok

echo   Masaustune kisayol kondu: {{UYGULAMA_ADI}}
echo.
